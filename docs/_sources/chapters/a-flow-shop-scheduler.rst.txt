==========================================
500 Lines or Less \| A Flow Shop Scheduler
==========================================
...
``_
.. rubric:: A Flow Shop Scheduler
   :name: a-flow-shop-scheduler

.. rubric:: Dr. Christian Muise
   :name: dr.-christian-muise
   :class: author
...
*`Dr. Christian Muise`_ is a Research Fellow with the `MERS group`_ at
...
well as celtic music, carving, soccer, and coffee.*

.. rubric:: A Flow Shop Scheduler
   :name: a-flow-shop-scheduler

*Flow shop scheduling* is one of the most challenging and well-studied
...
flow shop scheduling solver that uses a technique called *local search*.
...
example, each car is a new *job* and each part for the car is called a
*task*. Every job will have the same sequence of tasks to complete.
...
(Typically, this total time is referred to as the *makespan*.) This
problem has many applications, but is most related to optimizing
production facilities.

Every flow shop problem consists of \\(n\\) machines and \\(m\\) jobs.
...
1. The completion of the task for machine \\(i\\) in job \\(j-1\\)
   (i.e., the most recent task on the same machine), or

2. The completion of the task for machine \\(i-1\\) in job \\(j\\)
   (i.e., the most recent task on the same job)
...
valid solution, and the optimal solution will correspond to *some*
...
follows, we refer to a permutation of the jobs as a *candidate*.
...
is 5 (`Figure 9.1`_); on the other hand, if we order job 1 before job 2,
the makespan is only 4 (`Figure 9.2`_).
...
to focus on next, we define what is known as a *neighbourhood*: the set
...
fact what we do below).

To use local search formally, we must answer a few questions:

1. What solution should we start with?
2. Given a solution, what are the neighbouring solutions that we should
   consider?
3. Given the set of candidate neighbours, which one should we consider
   moving to next?

The following three sections address these questions in turn.

.. rubric:: General Solver
   :name: general-solver
...
``TIME_INCREMENT`` setting will be used as part of the dynamic strategy
selection, and the ``MAX_LNS_NEIGHBOURHOODS`` setting will be used as
...
following code is used as the ``__main__`` method for the solver file,
...
We will describe the parsing of Taillard problem files shortly. (The
files are `available online`_.)

The ``solve`` method expects the ``data`` variable to be a list of
integers containing the activity durations for each job. The ``solve``
method starts by initializing a global set of strategies (to be
described below). The key is that we use ``strat_*`` variables to
maintain statistics on each of the strategies. This aids in selecting
the strategy dynamically during the solving process.
...
*every* permutation is a valid solution, and at least one will have the
...
sufficient to know that the strategy provides a ``neighbourhood``
function and a ``heuristic`` function. The former gives us a set of
*next candidates* to consider while the latter chooses the *best
candidate* from the set. From these functions, we have a new permutation
(``perm``) and a new makespan result (``res``):
...
see below how ``compile_solution`` works, but for now it suffices to
know that a 2D array is returned and the element at ``[-1][-1]``
corresponds to the start time of the final job in the schedule:
...
.. rubric:: Parsing Problems
   :name: parsing-problems
...
by the solving code above. (Every item in ``data`` should correspond to
a particular job.)
...
.. rubric:: Compiling Solutions
   :name: compiling-solutions
...
``compile_solution`` function to convert a permutation to precise times.
As input, the function takes in the data for the problem (giving us the
duration of every task) and a permutation of jobs.
...
.. rubric:: Neighbourhoods
   :name: neighbourhoods

The idea behind local search is to move *locally* from one solution to
other solutions nearby. We refer to the *neighbourhood* of a given
solution as the other solutions that are local to it. In this section,
we detail four potential neighbourhoods, each of increasing complexity.
...
permutation. By using the ``combinations`` function from the
``itertools`` package, we can easily iterate through every pair of
...
permutations that are very similar to the one we began with.
...
swapping them in every way possible. We take in a value ``size`` which
is the number of jobs we consider: the ``size`` most idle jobs. The
...
we make use of the ``permutations`` function from the ``itertools``
package.
...
*Large Neighbourhood Search* (LNS). Intuitively, LNS works by
...
considered through the ``MAX_LNS_NEIGHBOURHOODS`` parameter, as the
...
consider swapping using the ``combinations`` function of the
``itertools`` package:
...
Next, we iterate through the subsets to find the best permutation of
...
If we were to set the ``size`` parameter to be equal to the number of
...
to around 3 or 4; anything larger would cause the ``neighbours_LNS``
function to take a prohibitive amount of time.
...
The first heuristic that we consider is ``heur_random``. This heuristic
...
The next heuristic ``heur_hillclimbing`` uses the other extreme. Rather
than randomly selecting a candidate, it selects the candidate that has
the best makespan. Note that the list ``scores`` will contain tuples of
the form ``(make,perm)`` where ``make`` is the makespan value for
permutation ``perm``. Sorting such a list places the tuple with the best
...
The final heuristic, ``heur_random_hillclimbing``, combines both the
...
one. The ``heur_random_hillclimbing`` heuristic returns a "pretty good"
...
corresponds to the candidate that the heuristic selects.
...
.. rubric:: Dynamic Strategy Selection
...
dynamically to those strategies that work best. For us, a *strategy* is
...
``partial`` function from the ``functools`` package to partially assign
...
of the strategies, but *weight the selection* based on how well the
strategy has performed. We describe the weighting below, but for the
``pick_strategy`` function, we need only a list of strategies and a
...
chosen. This technique, sometimes referred to as *roulette wheel
selection*, will randomly pick a strategy for us and give a greater
chance to those strategies with higher weight.
...
at regularly timed intervals (defined with the ``TIME_INCREMENT``
variable):
...
Recall that ``strat_improvements`` stores the sum of all improvements
that a strategy has made while ``strat_time_spent`` stores the time that
...
small amount of time as a default value.
...
``DEBUG_SWITCH`` flag is set), and we reset the ``strat_improvements``
and ``strat_time_spent`` variables for the next interval.
...
solution that is *good enough*. With local search we can move from one
solution to another, aiming to find one of good quality.
...
.. _: /en/index.html
.. _Dr. Christian Muise: http://haz.ca
.. _MERS group: http://groups.csail.mit.edu/mers/
.. _MIT's CSAIL: http://www.csail.mit.edu/
.. _Figure 9.1: #figure-9.1
.. _Figure 9.2: #figure-9.2
.. _available online: http://mistic.heig-vd.ch/taillard/problemes.dir/ordonnancement.dir/ordonnancement.html

.. |Figure 9.1 - Flow Shop Example 1| image:: flow-shop-images/example1.png
.. |Figure 9.2 - Flow Shop Example 2| image:: flow-shop-images/example2.png
