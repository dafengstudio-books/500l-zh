...
``_
.. rubric:: Optical Character Recognition (OCR)
   :name: optical-character-recognition-ocr

.. rubric:: Marina Samuel
   :name: marina-samuel
   :class: author
...
.. rubric:: What is Artificial Intelligence?
   :name: what-is-artificial-intelligence

  While Turing’s definition of intelligence sounds reasonable, at the
...
following examples, as well as `many others`_:

-  Logical and probabilistic deduction and inference based on some
   predefined knowledge of a world. e.g. `Fuzzy inference`_ can help a
...
-  Heuristic search. e.g. Searching can be used to find the best
   possible next move in a game of chess by searching all possible moves
   and choosing the one that most improves your position
...
systems with unlabelled data are called *unsupervised* algorithms and
those that train with labelled data are called *supervised*. Many ML
algorithms and techniques exist for creating OCR systems, of which ANNs
are one approach.

.. rubric:: Artificial Neural Networks
   :name: artificial-neural-networks

.. rubric:: What Are ANNs?
   :name: what-are-anns
...
inspired by neural networks found in a biological brain. `Hebbian
Theory`_ explains how these networks can learn to identify patterns by
physically altering their structure and link strengths. Similarly, a
typical ANN (shown in `Figure 15.1`_) has connections between nodes that
have a weight which is updated as the network learns. The nodes labelled
"+1" are called *biases*. The leftmost blue column of nodes are *input
nodes*, the middle column contains *hidden nodes*, and the rightmost
column contains *output nodes*. There may be many columns of hidden
nodes, known as *hidden layers*.
...
The values inside all of the circular nodes in `Figure 15.1`_ represent
...
\\[ a^{(2)}\_2 = f(w^{(1)}\_{21}x\_1 + w^{(1)}\_{22}x\_2 + b^{(1)}\_{2})
\\]

where \\(f(.)\\) is known as the *activation function* and \\(b\\) is
the *bias*. An activation function is the decision-maker for what type
...
details on both of these in `Designing a Feedforward ANN
(``neural_network_design.py``)`_.
...
one commonly used algorithm is called *backpropagation*. The OCR system
we will implement in this chapter will use backpropagation.

.. rubric:: How Do We Use ANNs?
   :name: how-do-we-use-anns
...
with labelled data, as mentioned in `What is Artificial Intelligence?`_.
...
large data set of drawn digits and associated values.
...
algorithm on our data, we would `split the data`_ into 50% for training,
...
and 25% for validating that it was trained well.

The purpose of identifying the accuracy of the ANN is two-fold. First,
it is to avoid the problem of *overfitting*. Overfitting occurs when the
...
.. rubric:: Design Decisions in a Simple OCR System
   :name: design-decisions-in-a-simple-ocr-system
...
.. rubric:: A Simple Interface (``ocr.html``)
   :name: a-simple-interface-ocr.html
...
.. rubric:: An OCR Client (``ocr.js``)
   :name: an-ocr-client-ocr.js
...
easier to see. Here we have a blue grid generated by ``drawGrid()``.
...
sent to the server. For simplicity, we can have an array called ``data``
which labels an uncoloured, black pixel as ``0`` and a coloured white
pixel as ``1``. We also need some mouse listeners on the canvas so we
know when to call ``fillSquare()`` to colour a pixel white while a user
is drawing a digit. These listeners should keep track of whether we are
in a drawing state and then call ``fillSquare()`` to do some simple math
...
relatively straight forward ``train()`` function that does some error
checking on the data to be sent, adds it to ``trainArray`` and sends it
off by calling ``sendData()``.
...
An interesting design worth noting here is the use of
``trainingRequestCount``, ``trainArray``, and ``BATCH_SIZE``. What’s
happening here is that ``BATCH_SIZE`` is some pre-defined constant for
...
``ocr.html`` page training the system), or if another layer existed in
the client that takes scanned drawn digits and translated them to pixels
to train the network, a ``BATCH_SIZE`` of 1 would result in many,
...
We will also need a ``test()`` function. Similar to ``train()``, it
...
.. rubric:: A Server (``server.py``)
   :name: a-server-server.py
...
among programmers; KNPLabs summarizes the issues `with humour`_.

Another consideration is whether to send the "train" vs. "predict"
requests to different endpoints (e.g. ``http://localhost/train`` and
``http://localhost/predict``) or the same endpoint which then processes
...
server error was actually caused by a bad request.

Finally, we need to decide when and where to initialize the OCR system.
A good approach would be to initialize it within ``server.py`` but
...
.. rubric:: Designing a Feedforward ANN (``neural_network_design.py``)
   :name: designing-a-feedforward-ann-neural_network_design.py
...
constraints. StatSoft provides a `nice list`_ of common activation
...
going back to how the output of a node is computed in `Figure 15.1`_.
...
a bias adds a constant to our linear function input to \\(f(.)\\).

Adding the bias therefore allows for a shift in the \\(y\\)-intercept
...
ANN or would need more hidden nodes to make more accurate predictions.
...
increments of 5. We then call the ``test()`` function.
...
``neural_network_design.py`` looks like the following:
...
that case it would be better to choose 45 hidden nodes instead of 15.

.. rubric:: Core OCR Functionality
   :name: core-ocr-functionality
...
.. rubric:: Training via Backpropagation (``ocr.py``)
   :name: training-via-backpropagation-ocr.py

We use the backpropagation algorithm to train our ANN. It consists of 4
main steps that are repeated for every sample in the training set,
updating the ANN weights each time.
...
and store them in matrices ``theta1``, ``theta2``, ``input_layer_bias``,
and ``hidden_layer_bias``. Since every node in a layer links to every
...
columns for our 20x20 pixel inputs and ``num_hidden_nodes`` rows.
Likewise, ``theta2`` represents the links between the hidden layer and
output layer. It has ``num_hidden_nodes`` columns and ``NUM_DIGITS``
(``10``) rows. The other two vectors (1 row), ``input_layer_bias`` and
``hidden_layer_bias`` represent the biases.
...
The second step is *forward propagation*, which is essentially computing
the node outputs as described in `What Are ANNs?`_, layer by layer
starting from the input nodes. Here, ``y0`` is an array of size 400 with
the inputs we wish to use to train the ANN. We multiply ``theta1`` by
``y0`` transposed so that we have two matrices with sizes
``(num_hidden_nodes x 400) * (400 x 1)`` and have a resulting vector of
outputs for the hidden layer of size num\_hidden\_nodes. We then add the
bias vector and apply the vectorized sigmoid activation function to this
output vector, giving us ``y1``. ``y1`` is the output vector of our
hidden layer. The same process is repeated again to compute ``y2`` for
the output nodes. ``y2`` is now our output layer vector with values
representing the likelihood that their index is the drawn number. For
example if someone draws an 8, the value of ``y2`` at the 8th index will
...
be drawn as the 8. ``y2`` becomes more accurate with each additional
...
The third step is *back propagation*, which involves computing the
...
``actual_vals``, with a ``1`` at the index of the digit that represents
the value of the drawn digit and ``0``\ s otherwise. The vector of
errors at the output nodes, ``output_errors``, is computed by
subtracting the actual output vector, ``y2``, from ``actual_vals``. For
...
.. rubric:: Testing a Trained Network (``ocr.py``)
   :name: testing-a-trained-network-ocr.py
...
start by computing the output of the ANN, ``y2``, exactly the way we did
in step 2 of backpropagation. Then we look for the index in the vector
with the maximum value. This index is the digit predicted by the ANN.
...
.. rubric:: Other Design Decisions (``ocr.py``)
   :name: other-design-decisions-ocr.py

Many resources are available online that go into greater detail on the
implementation of backpropagation. One good resource is from a `course
by the University of Willamette`_. It goes over the steps of
...
As mentioned in the introduction of `Design Decisions in a Simple OCR
System`_, persisting the weights of the ANN means we do not lose the
...
.. _: /en/index.html
.. _many others: http://www-formal.stanford.edu/jmc/whatisai/node2.html
.. _Fuzzy inference: http://www.cs.princeton.edu/courses/archive/fall07/cos436/HIDDEN/Knapp/fuzzy004.htm
.. _Hebbian Theory: http://www.nbb.cornell.edu/neurobio/linster/BioNB420/hebb.pdf
.. _Figure 15.1: #figure-15.1
.. _Designing a Feedforward ANN (``neural_network_design.py``): #sec.ocr.feedforward
.. _What is Artificial Intelligence?: #sec.ocr.ai
.. _split the data: http://www-group.slac.stanford.edu/sluo/Lectures/stat_lecture_files/sluo2006lec7.pdf
.. _with humour: https://knpuniversity.com/screencast/rest/put-versus-post
.. _nice list: http://www.fmi.uni-sofia.bg/fmi/statist/education/textbook/eng/glosa.html
.. _What Are ANNs?: #sec.ocr.ann
.. _course by the University of Willamette: http://www.willamette.edu/~gorr/classes/cs449/backprop.html
.. _Design Decisions in a Simple OCR System: #sec.ocr.decisions

.. |Figure 15.1 - An Artificial Neural Network| image:: ocr-images/ann.png
