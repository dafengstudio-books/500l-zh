==================================================
500 Lines or Less \| Making Your Own Image Filters
==================================================
...
``_
.. rubric:: Making Your Own Image Filters
   :name: making-your-own-image-filters
...
*Cate left the tech industry and spent a year finding her way back
...
at `Accidentally in Code`_ and is `@catehstn`_ on Twitter.*

.. rubric:: A Brilliant Idea (That Wasn’t All That Brilliant)
   :name: a-brilliant-idea-that-wasnt-all-that-brilliant
...
progression through the seasons.
...
images, though: in a layout called the `Sunflower layout`_. It’s the
most efficient way to lay out circles.
...
color, and `finished my visualization`_. That is when I discovered that
...
image), and it took hundreds of images to make something cool (`Figure
11.1`_).
...
I used a framework called `Processing`_ because I was familiar with it
...
.. rubric:: Photographs, the Old Way
   :name: photographs-the-old-way
...
When talking an old-fashioned photograph — with film — the light hits
...
Then, the `development process`_ converts the silver salts to metallic
...
.. rubric:: Photographs, the Digital Way
   :name: photographs-the-digital-way
...
In `Figure 11.2`_, we see a high-resolution picture of some blow-up
animals taken at MoMA in NYC. `Figure 11.3`_ is the same image blown up,
...
See how it's so blurry? We call that *pixelation*, which means the image
...
``Integer.toHexString`` in Java, we get hex colors:
...
In `Figure 11.4`_, we have a picture of our app running. It’s very much
...
right. Some things we can do:

-  Adjust the RGB filters.
-  Adjust the “hue tolerance”.
-  Set the dominant hue filters, to either show or hide the dominant
   hue.
...
For this tutorial, I use Processing in Eclipse by adding ``core.jar`` to
...
replace the file chooser with something else.

There are detailed instructions with screenshots in the project's
`repository`_. If you are familiar with Eclipse and Java already you may
not need them.

.. rubric:: Processing Basics
   :name: processing-basics

.. rubric:: Size and Color
   :name: size-and-color

We don’t want our app to be a tiny grey window, so the two essential
methods that we will start by overriding are ```setup()```_, and
```draw()```_. The ``setup()`` method is only called when the app
starts, and is where we do things like set the size of the app window.
The ``draw()`` method is called for every animation, or after some
action can be triggered by calling ``redraw()``. (As covered in the
...
in this case we don’t want animation\ `:sup:`1``_, we want to respond to
key presses. To prevent animation (which would be a drag on performance)
we will call ```noLoop()```_ from setup. This means that ``draw()`` will
only be called immediately after ``setup()``, and whenever we call
``redraw()``.
...
the constants in ``WIDTH`` and ``HEIGHT``, to see different sizes.

``background(0)`` specifies a black background. Try changing the number
passed to ``background()`` and see what happens — it’s the alpha value,
and so if you only pass one number in, it is always greyscale.
Alternatively, you can call ``background(int r, int g, int b)``.

.. rubric:: PImage
   :name: pimage

The `PImage object`_ is the Processing object that represents an image.
...
call ```selectInput()```_, and implement a callback (which must be
public).
...
.. rubric:: Responding to Key Presses
   :name: responding-to-key-presses
...
We create "seams" that allow us to break something up from its amorphous
...
library. For mocking I used Mockito. You can download `Mockito`_ and add
the JAR to your buildpath in the same way you added ``core.jar``. I
created two helper classes that make it possible to mock and test the
app (otherwise we can’t test behavior involving ``PImage`` or
``PApplet`` methods).

``IFAImage`` is a thin wrapper around PImage. ``PixelColorHelper`` is a
...
after all — not a unit testing framework for Processing!

A class called ``ImageState`` forms the "model" of this application,
removing as much logic from the class extending ``PApplet`` as possible,
for better testability. It also makes for a cleaner design and
separation of concerns: the ``App`` controls the interactions and the
UI, not the image manipulation.

.. rubric:: Do-It-Yourself Filters
   :name: do-it-yourself-filters
...
By applying different filters to this image `Figure 11.5`_ (taken on a
...
more green the tree becomes when the red filter is applied.
...
-  Set the filter. (You can combine red, green and blue filters as in
...
array starting top-left and moving `left to right, top to bottom`_. The
array indices for a 4x4 image are shown here:
...
.. rubric:: HSB or HSV colors
   :name: hsb-or-hsv-colors
...
.. rubric:: Extracting the Dominant Hue from an Image
   :name: extracting-the-dominant-hue-from-an-image
...
to the HSB color space using `colorMode`_.
...
``colorMode(HSB, 120)`` we have just made our hue detection a bit less
...
pixels, as it requires action on each one.
...
the picture (`Figure 11.6`_).

.. raw:: html

   <div class="center figure">

\ |Figure 11.6 - Dominant hue versus size of range (number of buckets)
used|
...
Once we’ve extracted the "dominant" hue, we can choose to either show or
...
on the brightness. `Figure 11.7`_ shows the dominant hue determined
...
Alternatively, we can hide the dominant hue. In `Figure 11.8`_, the
...
Processing has some `built-in methods`_ that support the manipulation of
images; for example, ``invert`` and ``blur``.
...
it, with the corresponding value of the `filter matrix`_. There are some
special matrices of specific values that sharpen images.

.. rubric:: Architecture
   :name: architecture

There are three main components to the app (`Figure 11.9`_).

.. rubric:: The App
   :name: the-app-1

The app consists of one file: ``ImageFilterApp.java``. This extends
``PApplet`` (the Processing app superclass) and handles layout, user
...
``IFAImage`` is a wrapper around ``PImage`` for testability. (``PImage``
contains a number of final methods which cannot be mocked.) Finally,
``ImageState.java`` is the object which describes the state of the image
...
abstracts out final ``PApplet`` methods for pixel colors for
testability.
...
.. rubric:: Wrapper Classes and Tests
   :name: wrapper-classes-and-tests

Briefly mentioned above, there are two wrapper classes (``IFAImage`` and
``PixelColorHelper``) that wrap library methods for testability. This is
because, in Java, the keyword "final" indicates a method that cannot be
overridden or hidden by subclasses, which means they cannot be mocked.

``PixelColorHelper`` wraps methods on the applet. This means we need to
pass the applet in to each method call. (Alternatively, we could make it
a field and set it on initialization.)
...
``IFAImage`` is a wrapper around ``PImage``, so in our app we don’t
initialize a ``PImage``, but rather an ``IFAImage`` — although we do
have to expose the ``PImage`` so that it can be rendered.
...
Finally, we have our simple container class, ``HSBColor``. Note that it
...
in
`Processing <https://www.processing.org/reference/color_datatype.html>`__
and in `Java itself`_. Without going too much into the details of these,
...
okay if we did want to use Java’s ``awt.Color``; however `awt GUI
components cannot be used in Processing`_, so for our purposes creating
...
.. rubric:: ColorHelper and Associated Tests
   :name: colorhelper-and-associated-tests

``ColorHelper`` is where all the image manipulation lives. The methods
in this class could be static if not for needing a ``PixelColorHelper``.
...
use. We are using
`Mockito <http://docs.mockito.googlecode.com/hg/org/mockito/Mockito.html>`__
as our mock object framework.

To create a mock we use the ``@Mock`` annotation on an instance
...
visit the source folder for this project in the `*500 Lines or Less*
GitHub repository`_.
...
-  We mock ``PApplet``, ``IFAImage`` (created for expressly this
   purpose), and ``ImageColorHelper``.
-  Test methods are annotated with ``@Test``\ `:sup:`2``_. If you want
   to ignore a test (e.g., whilst debugging) you can add the annotation
   ``@Ignore``.
-  In ``setup()``, we create the pixel array and have the mock image
   always return it.
-  Helper methods make it easier to set expectations for recurring tasks
   (e.g., ``set*ForPixel()``).

.. rubric:: Image State and Associated Tests
   :name: image-state-and-associated-tests

``ImageState`` holds the current "state" of the image — the image
itself, and the settings and filters that will be applied. We'll omit
the full implementation of ``ImageState`` here, but we'll show how it
...
-  We exposed a protected initialization method ``set`` for testing that
   helps us quickly get the system under test into a specific state.
-  We mock ``PApplet``, ``ColorHelper``, and ``IFAImage`` (created
   expressly for this purpose).
-  This time we use a helper (``assertState()``) to simplify asserting
   the state of the image.

.. rubric:: Measuring test coverage
   :name: measuring-test-coverage

I use `EclEmma`_ to measure test coverage within Eclipse. Overall for
the app we have 81% test coverage, with none of ``ImageFilterApp``
covered, 94.8% for ``ImageState``, and 100% for ``ColorHelper``.

.. rubric:: ImageFilterApp
   :name: imagefilterapp
...
.. rubric:: The Value of Prototyping
   :name: the-value-of-prototyping
...
more time on corner cases than refining algorithms.
...
periods of time locked away in a dark room, cursing intermittently.
...
moment and milestone the following morning.
...
#. 
   ...
   If we wanted to create an animated sketch we would not call
   ``noLoop()`` (or, if we wanted to start animating later, we would
   call ``loop()``). The frequency of the animation is determined by
   ``frameRate()``.\ `↩`_
...
#. 
   ...
   Method names in tests need not start with ``test`` as of JUnit 4, but
   habits are hard to break.\ `↩ <#fnref2>`__
...
.. _: /en/index.html
.. _Accidentally in Code: http://www.catehuston.com/blog/
.. _@catehstn: https://twitter.com/catehstn
.. _Sunflower layout: http://www.catehuston.com/applets/Sunflower/index.html
.. _finished my visualization: http://www.catehuston.com/blog/2013/09/02/visualising-a-photo-series/
.. _Figure 11.1: #figure-11.1
.. _Processing: https://processing.org/
.. _development process: http://photography.tutsplus.com/tutorials/step-by-step-guide-to-developing-black-and-white-t-max-film--photo-2580
.. _Figure 11.2: #figure-11.2
.. _Figure 11.3: #figure-11.3
.. _Figure 11.4: #figure-11.4
.. _repository: https://github.com/aosabook/500lines/blob/master/image-filters/SETUP.MD
.. _``setup()``: http://processing.org/reference/setup_.html
.. _``draw()``: http://processing.org/reference/draw_.html
.. _`:sup:`1``: #fn1
.. _``noLoop()``: http://www.processing.org/reference/noLoop_.html
.. _PImage object: http://processing.org/reference/PImage.html
.. _``selectInput()``: http://www.processing.org/reference/selectInput_.html
.. _``keyPressed()``: https://www.processing.org/reference/keyPressed_.html
.. _Mockito: https://code.google.com/p/mockito/downloads/list
.. _Figure 11.5: #figure-11.5
.. _left to right, top to bottom: https://processing.org/tutorials/pixels/
.. _colorMode: http://processing.org/reference/colorMode_.html
.. _Figure 11.6: #figure-11.6
.. _Figure 11.7: #figure-11.7
.. _Figure 11.8: #figure-11.8
.. _built-in methods: https://www.processing.org/reference/filter_.html
.. _filter matrix: http://lodev.org/cgtutor/filtering.html
.. _Figure 11.9: #figure-11.9
.. _Java itself: https://docs.oracle.com/javase/7/docs/api/java/awt/Color.html
.. _awt GUI components cannot be used in Processing: http://processing.org/reference/javadoc/core/processing/core/PApplet.html
.. _*500 Lines or Less* GitHub repository: https://github.com/aosabook/500lines/tree/master/image-filters
.. _`:sup:`2``: #fn2
.. _EclEmma: http://www.eclemma.org/installation.html#marketplace
.. _Show & Hide: http://showandhide.com
.. _↩: #fnref1

.. |Figure 11.1 - Sunflower layout| image:: image-filters-images/sunflower.jpg
.. |Figure 11.2 - Blow-up animals at MoMA NY| image:: image-filters-images/animals.jpg
.. |Figure 11.3 - Blow-up animals, blown up| image:: image-filters-images/pixelanimals.jpg
.. |Figure 11.4 - The App| image:: image-filters-images/app.jpg
.. |Figure 11.5 - Four (Simulated) Seasons in Frankfurt| image:: image-filters-images/frankfurt.jpg
.. |Figure 11.6 - Dominant hue versus size of range (number of buckets) used| image:: image-filters-images/hueranges.jpg
.. |Figure 11.7 - Showing dominant hue| image:: image-filters-images/showdominant.jpg
.. |Figure 11.8 - Hiding dominant hue| image:: image-filters-images/hidedominant.jpg
.. |Figure 11.9 - Architecture diagram| image:: image-filters-images/architecture.jpg
