...
*`Greg Wilson`_ is the founder of Software Carpentry, a crash course in
...
*Beautiful Code* and the first two volumes of *The Architecture of Open
Source Applications*. Greg received a PhD in Computer Science from the
...
consists of four 8-bit numbers, such as ``174.136.14.108``; the Domain
Name System (DNS) matches these numbers to symbolic names like
``aosabook.org`` that are easier for human beings to remember.
...
server sends some data in response (`Figure 22.1`_.) The data may be
...
understood, though, that text must have the parts shown in `Figure
22.2`_.
...
The HTTP method is almost always either "GET" (to fetch information) or
...
``/research/experiments.html``, but (and this is the crucial part) it's
...
the start of the body to signal the end of the headers.

One header, called ``Content-Length``, tells the server how many bytes
to expect to read in the body of the request.

HTTP responses are formatted like HTTP requests (`Figure 22.3`_):
...
The first is that it is *stateless*: each request is handled on its own,
...
``http://www.google.ca?q=Python`` asks Google to search for pages
related to Python: the key is the letter 'q', and the value is 'Python'.
The longer query
``http://www.google.ca/search?q=Python&amp;client=Firefox`` tells Google
...
plumbing that most people never want to care about. The `Requests`_
library is an easier-to-use alternative to ``urllib2``. Here's an
...
``request.get`` sends an HTTP GET request to a server and returns an
object containing the response. That object's ``status_code`` member is
the response's status code; its ``content_length`` member is the number
of bytes in the response data, and ``text`` is the actual data (in this
case, an HTML page).
...
Python standard library has a module called ``BaseHTTPServer`` that does
...
The library's ``BaseHTTPRequestHandler`` class takes care of parsing the
incoming HTTP request and deciding what method it contains. If the
method is GET, the class calls a method named ``do_GET``. Our class
``RequestHandler`` overrides this method to dynamically generate a
simple page: the text is stored in the class-level variable ``Page``,
which we send back to the client after sending a 200 response code, a
``Content-Type`` header telling the client to interpret our data as
HTML, and the page's length. (The ``end_headers`` method call inserts
the blank line that separates our headers from the page itself.)

But ``RequestHandler`` isn't the whole story: we still need the last
...
of ``BaseHTTPServer.HTTPServer`` with that address and the name of our
request handler class as parameters, then ask it to run forever (which
in practice means until we kill it with Control-C).
...
If we then go to ``http://localhost:8080`` with our browser, though, we
get this in our browser:
...
``/favicon.ico``, which it will display as an icon in the address bar if
it exists.

.. rubric:: Displaying Values
...
instance of the ``HTTPServer`` class with an address and this request
handler as parameters, then serves requests forever. If we run it and
send a request from a browser for
``http://localhost:8080/something.html``, we get:
...
Notice that we do *not* get a 404 error, even though the page
``something.html`` doesn't exist as a file on disk. That's because a web
...
Wikipedia page chosen at random, or whatever else we program it to.

.. rubric:: Serving Static Pages
   :name: serving-static-pages
...
``os.getcwd``). It combines this with the path provided in the URL
(which the library automatically puts in ``self.path``, and which always
starts with a leading '/') to get the path to the file the user wants.
...
on the other hand, it calls a helper method named ``handle_file`` to
read and return the contents. This method just reads the file and uses
our existing ``send_content`` to send it back to the client:
...
Note that we open the file in binary mode—the 'b' in 'rb'—so that Python
...
need to modify ``handle_error`` and ``send_content`` as follows:
...
but generate an error page instead. A ``ServerException`` is meant to
signal an internal error in the server code, i.e., something that *we*
got wrong. The error page created by ``handle_error``, on the other
hand, appears when the *user* got something wrong, i.e., sent us the URL
of a file that doesn't exist. `:sup:`1``_

.. rubric:: Listing Directories
...
directory for an ``index.html`` file to display, and only show a listing
of the directory's contents if that file is not present.

But building these rules into ``do_GET`` would be a mistake, since the
resulting method would be a long tangle of ``if`` statements controlling
...
stored in a list. Each case is an object with two methods: ``test``,
which tells us whether it's able to handle the request, and ``act``,
...
to serve up the ``index.html`` page for a directory if there is one, and
...
Here, the helper method ``index_path`` constructs the path to the
``index.html`` file; putting it in the case handler prevents clutter in
the main ``RequestHandler``. ``test`` checks whether the path is a
directory containing an ``index.html`` page, and ``act`` asks the main
request handler to serve that page.

The only change needed to ``RequestHandler`` is to add a
``case_directory_index_file`` object to our ``Cases`` list:
...
What about directories that don't contain ``index.html`` pages? The test
is the same as the one above with a ``not`` strategically inserted, but
what about the ``act`` method? What should it do?
...
It seems we've backed ourselves into a corner. Logically, the ``act``
method should create and return the directory listing, but our existing
code doesn't allow for that: ``RequestHandler.do_GET`` calls ``act``,
but doesn't expect or handle a return value from it. For now, let's add
a method to ``RequestHandler`` to generate a directory listing, and call
that from the case handler's ``act``:
...
.. rubric:: The CGI Protocol
   :name: the-cgi-protocol
...
The test is simple: does the file path end with ``.py``? If so,
``RequestHandler`` runs this program.
...
The full CGI protocol is much richer than this—in particular, it allows
...
...which is once again becoming rather tangled. ``RequestHandler``
initially had one method, ``handle_file``, for dealing with content. We
have now added two special cases in the form of ``list_dir`` and
``run_cgi``. These three methods don't really belong where they are,
...
shared by two or more handlers. When we're done, the ``RequestHandler``
...
collection of related services. ``RequestHandler`` and ``base_case``
don't make decisions or take actions; they provide tools that other
classes can use to do those things.
...
``RequestHandler`` (to insert the case handler in the ``Cases`` list),
...
``BaseHTTPRequestHandler`` class have allowed us to ignore the details
of handling socket connections and parsing HTTP requests.

These ideas are generally useful; see if you can find ways to use them
...
#. 
   ...
   We're going to use ``handle_error`` several times throughout this
   chapter, including several cases where the status code ``404`` isn't
...
#. 
   ...
   Our code also uses the ``popen2`` library function, which has been
   deprecated in favor of the ``subprocess`` module. However, ``popen2``
...
.. _: /en/index.html
.. _Greg Wilson: https://twitter.com/gvwilson
.. _Figure 22.1: #figure-22.1
.. _Figure 22.2: #figure-22.2
.. _Figure 22.3: #figure-22.3
.. _Requests: https://pypi.python.org/pypi/requests
.. _`:sup:`1``: #fn1
.. _`:sup:`2``: #fn2
.. _↩: #fnref1

.. |Figure 22.1 - The HTTP Cycle| image:: web-server-images/http-cycle.png
.. |Figure 22.2 - An HTTP Request| image:: web-server-images/http-request.png
.. |Figure 22.3 - An HTTP Response| image:: web-server-images/http-response.png
