...
``_
.. rubric:: Clustering by Consensus
   :name: clustering-by-consensus

.. rubric:: Dustin J. Mitchell
...
*Dustin is an open source software developer and release engineer at
...
Python. Find him as `@djmitche`_ on GitHub or at*
...
Instead, the service is *distributed* over multiple servers, with each
running a separate instance of exactly the same code. Users can then
contact any server to perform an operation.
...
.. rubric:: Distributed State Machines
...
problem of *consensus*, and we'll address it with a derivative of the
Paxos algorithm.

.. rubric:: Consensus by Paxos
   :name: consensus-by-paxos
...
Part-Time Parliament"\ `:sup:`1``_. Lamport's paper has a great deal
...
algorithm that we have adapted in this implementation.
...
A ballot begins with the proposer sending a ``Prepare`` message with the
ballot number *N* to the acceptors and waiting to hear from a majority
(`Figure 3.1`_.)

The ``Prepare`` message is a request for the accepted value (if any)
with the highest ballot number less than *N*. Acceptors respond with a
``Promise`` containing any value they have already accepted, and
promising not to accept any ballot numbered less than *N* in the future.
If the acceptor has already made a promise for a larger ballot number,
it includes that number in the ``Promise``, indicating that the proposer
...
sends an ``Accept`` message, including the ballot number and value, to
...
the value from the highest-numbered promise.

Unless it would violate a promise, each acceptor records the value from
the ``Accept`` message as accepted and replies with an ``Accepted``
message. The ballot is complete and the value decided when the proposer
has heard its ballot number from a majority of acceptors.

Returning to the example, initially no other value has been accepted, so
the acceptors all send back a ``Promise`` with no value, and the
proposer sends an ``Accept`` containing its value, say:
...
``'Dustin J. Mitchell'``), the acceptors will simply not accept it. If
that ballot has a larger ballot number, then the ``Promise`` from the
acceptors will inform the proposer about Michael's $100.00 deposit
operation, and the proposer will send that value in the ``Accept``
message instead of the transfer to Dustin. The new ballot will be
accepted, but in favor of the same value as the first ballot.
...
-  Proposer A performs the ``Prepare``/``Promise`` phase for ballot
   number 1.
-  Before Proposer A manages to get its proposal accepted, Proposer B
   performs a ``Prepare``/``Promise`` phase for ballot number 2.
-  When Proposer A finally sends its ``Accept`` with ballot number 1,
   the acceptors reject it because they have already promised ballot
   number 2.
-  Proposer A reacts by immediately sending a ``Prepare`` with a higher
   ballot number (3), before Proposer B can send its ``Accept`` message.
-  Proposer B's subsequent ``Accept`` is rejected, and the process
   repeats.
...
numbers for all slots, and performing the ``Prepare``/``Promise`` phase
for all slots at once.

.. rubric:: Paxos Made Pretty Hard
   :name: paxos-made-pretty-hard
...
The ``Prepare``/``Promise`` phase can function as a kind of leader
...
execute the ``Accept``/``Accepted`` phase directly without repeating the
...
made. For example, if the initial ``Prepare`` message is lost and
doesn't reach the acceptors, then the proposer will wait for a
``Promise`` message that will never arrive. Fixing this requires
...
a ``Decision`` message can take care of this for the normal case. If the
...
.. rubric:: Introducing Cluster
   :name: introducing-cluster

The *Cluster* library in this chapter implements a simple form of
...
the protocol. Each is a subclass of ``Role``.
...
The roles that a cluster node has are glued together by the ``Node``
...
the message type with a ``do_`` prefix. These ``do_`` methods receive
the message's attributes as keyword arguments for easy access. The
``Node`` class also provides a ``send`` method as a convenience, using
``functools.partial`` to supply some arguments to the same methods of
the ``Network`` class.
...
.. rubric:: Application Interface
   :name: application-interface

The application creates and starts a ``Member`` object on each cluster
...
then runs the protocol (via ``Network.run``) in a separate thread.

The application interacts with the cluster through the ``invoke``
method, which kicks off a proposal for a state transition. Once that
proposal is decided and the state machine runs, ``invoke`` returns the
machine's output. The method uses a simple synchronized ``Queue`` to
wait for the result from the protocol thread.
...
The ``Acceptor`` implements the acceptor role in the protocol, so it
...
``Prepare`` and ``Accept`` messages according to the protocol. The
result is a short class that is easy to compare to the protocol.
...
The ``Replica`` class is the most complicated role class, as it has a
few closely related responsibilities:
...
sending a ``Propose`` message to the current leader (`Figure 3.2`_.)
...
``Decision`` messages represent slots on which the cluster has come to
...
*decided* slots, on which the cluster has agreed, from *committed*
...
sends an ``Invoked`` message back to the requester with the result of
the operation.
...
eventually decided, then the state machine does nothing for that slot.
...
``Propose`` messages to it. There is a surprising amount of subtlety
required to get this right, as we'll see later. Each replica tracks the
active leader using three sources of information.

When the leader role becomes active, it sends an ``Adopted`` message to
the replica on the same node (`Figure 3.3`_.)
...
When the acceptor role sends a ``Promise`` to a new leader, it sends an
``Accepting`` message to its local replica (`Figure 3.4`_.)
...
The active leader sends ``Active`` messages as a heartbeat (`Figure
3.5`_.) If no such message arrives before the ``LEADER_TIMEOUT``
expires, the replica assumes the leader is dead and moves on to the next
leader. In this case, it's important that all replicas choose the *same*
...
``Join`` message (`Figure 3.6`_.) The replica responds with a
``Welcome`` message containing its most recent state, allowing the new
node to come up to speed quickly.
...
.. rubric:: Leader, Scout, and Commander
   :name: leader-scout-and-commander

The leader's primary task is to take ``Propose`` messages requesting new
ballots and produce decisions. A leader is "active" when it has
successfully carried out the ``Prepare``/``Promise`` portion of the
protocol. An active leader can immediately send an ``Accept`` message in
...
response to receiving a ``Propose`` when it is inactive (`Figure 3.7`_.)
The scout sends (and re-sends, if necessary) a ``Prepare`` message, and
collects ``Promise`` responses until it has heard from a majority of its
peers or until it has been preempted. It communicates back to the leader
with ``Adopted`` or ``Preempted``, respectively.
...
proposal (`Figure 3.8`_.) Like a scout, a commander sends and re-sends
``Accept`` messages and waits for a majority of acceptors to reply with
``Accepted``, or for news of its preemption. When a proposal is
accepted, the commander broadcasts a ``Decision`` message to all nodes.
...
messages within a node. When *all* ``Decision`` messages were lost, the
protocol could not proceed. The replica continued to re-transmit
``Propose`` messages, but the leader ignored them as it already had a
...
sending ``Join`` messages to each peer in turn until it receives a
``Welcome``. Bootstrap's communication diagram is shown above in
`Replica`_.
...
"startup" phase, waiting for information from the ``Welcome`` message.
...
passing the initial state to their constructors.
...
to a ``Join`` message. But how does the cluster get started? One option
...
``Join`` times out. More importantly, in the event of a network
...
``Join`` messages from a majority of its peers, then sends a ``Welcome``
...
Seed emulates the ``Join``/``Welcome`` part of the bootstrap/replica
...
The role class simply sends ``Invoke`` messages to the local replica
until it receives a corresponding ``Invoked``. See the "Replica"
section, above, for this role's communication diagram.
...
-  Replica -- manage the distributed state machine: submitting
   proposals, committing decisions, and responding to requesters
-  Leader -- lead rounds of the Multi-Paxos algorithm
-  Scout -- perform the ``Prepare``/``Promise`` portion of the
   Multi-Paxos algorithm for a leader
-  Commander -- perform the ``Accept``/``Accepted`` portion of the
   Multi-Paxos algorithm for a leader
...
The ``Network`` class provides a simple simulated network with these
capabilities and also simulates packet loss and message propagation
delays.

Timers are handled using Python's ``heapq`` module, allowing efficient
selection of the next event. Setting a timer involves pushing a
``Timer`` object onto the heap. Since removing items from a heap is
inefficient, cancelled timers are left in place but marked as cancelled.
...
again use ``functools.partial`` to set up a future call to the
destination node's ``receive`` method with appropriate arguments.
...
.. rubric:: Debugging Support
   :name: debugging-support

When developing a complex system such as this, the bugs quickly
transition from trivial, like a simple ``NameError``, to obscure
...
The most important debugging feature in Cluster is a *deterministic*
...
Of course, much of that detail is in the messages exchanged by the nodes
...
as well as the simulated timestamp injected via the ``SimTimeLogger``
class.
...
aliasing error caused all replicas to share the same ``decisions``
...
of the art of debugging. In this code from ``Replica.do_Decision``, the
problem was that the ``Decision`` for the next slot to commit was being
ignored because it was already in ``self.decisions``. The underlying
assumption being violated was that the next slot to be committed was not
yet decided. Asserting this at the beginning of ``do_Decision``
...
unit (the ``Leader`` class). It follows the well-known "arrange, act,
...
classes. For example, the constructor for ``Leader`` looks like this:
...
The ``spawn_scout`` method (and similarly, ``spawn_commander``) creates
the new role object with ``self.scout_cls``:
...
The magic of this technique is that, in testing, ``Leader`` can be given
fake classes and thus tested separately from ``Scout`` and
``Commander``.

.. rubric:: Interface Correctness
   :name: interface-correctness
...
verify the format of the ``accepted`` attribute of the ``Promise``
message, and the unit tests for the scout role supply well-formatted
values for the attribute. Neither test checks that those formats match.
...
large part of the interface.

For specific issues such as the format of ``accepted_proposals``, both
the real and test data can be verified using the same function, in this
case ``verifyPromiseAccepted``. The tests for the acceptor use this
method to verify each returned ``Promise``, and the tests for the scout
use it to verify every fake ``Promise``.
...
A common approach to this sort of problem is "fuzz testing": running the
code repeatedly with randomly changing inputs until something breaks.
When something *does* break, all of the debugging support becomes
...
to become active every time it gets a ``Propose`` message.

If the whole cluster doesn't agree on which member is the active leader,
there's trouble: different replicas send ``Propose`` messages to
...
possible: when an acceptor sends a ``Promise``, chances are good that
the promised member will be the next leader. Failures are detected with
a heartbeat protocol.

.. rubric:: Further Extensions
   :name: further-extensions
...
In "pure" Multi-Paxos, nodes which fail to receive messages can be many
...
when a ``Decision`` message was lost, the replica would quickly find out
about the decision from one of its peers.

.. rubric:: Consistent Memory Usage
   :name: consistent-memory-usage
...
In the protocol definition, acceptors and replicas form the "memory" of
...
these decisions are transmitted between replicas in ``Welcome``
messages, making these messages enormous in a long-lived cluster.

One technique to address this issue is to periodically "checkpoint" each
...
*view*) can be changed by special view-change proposals. But the Paxos
...
Simple":
    ...
    We can allow a leader to get \\(\\alpha\\) commands ahead by letting
    ...
    \\(i\\)th state machine command. (Lamport, 2001)
...
\\(\\alpha\\) makes view changes slow to take effect.
...
-  tracking the view for each of the last \\(\\alpha\\) committed slots
   and correctly sharing this with new nodes,
...
Made Simple"\ `:sup:`2``_, our implementation added extensions that were
informed by several other resources. The role names were taken from
"Paxos Made Moderately Complex"\ `:sup:`3``_. "Paxos Made
Live"\ `:sup:`4``_ was helpful regarding snapshots in particular, and
`"Paxos Made Practical"`_ described view changes (although not of the
type described here.) Liskov's "From Viewstamped Replication to
Byzantine Fault Tolerance"\ `:sup:`5``_ provided yet another perspective
on view changes. Finally, a `Stack Overflow discussion`_ was helpful in
learning how members are added and removed from the system.
...
#. 
   ...
   Systems, 16(2):133–169, May 1998.\ `↩`_
...
#. 
   ...
   R. Van Renesse and D. Altinbuken, "Paxos Made Moderately Complex,"
   ACM Comp. Survey 47, 3, Article 42 (Feb. 2015)\ `↩ <#fnref3>`__
...
#. 
   ...
   B. Liskov, "From Viewstamped Replication to Byzantine Fault
   Tolerance," In *Replication*, Springer-Verlag, Berlin, Heidelberg
   121-149 (2010)`↩ <#fnref5>`__
...
.. _: /en/index.html
.. _@djmitche: http://github.com/djmitche
.. _`:sup:`1``: #fn1
.. _Figure 3.1: #figure-3.1
.. _``namedtuple``: https://docs.python.org/3/library/collections.html
.. _Figure 3.2: #figure-3.2
.. _Figure 3.3: #figure-3.3
.. _Figure 3.4: #figure-3.4
.. _Figure 3.5: #figure-3.5
.. _Figure 3.6: #figure-3.6
.. _Figure 3.7: #figure-3.7
.. _Figure 3.8: #figure-3.8
.. _Replica: #sec.cluster.replica
.. _`:sup:`2``: #fn2
.. _`:sup:`3``: #fn3
.. _`:sup:`4``: #fn4
.. _"Paxos Made Practical": http://www.scs.stanford.edu/~dm/home/papers/paxos.pdf
.. _`:sup:`5``: #fn5
.. _Stack Overflow discussion: http://stackoverflow.com/questions/21353312/in-part-time-parliament-why-does-using-the-membership-from-decree-n-3-work-to
.. _↩: #fnref1

.. |Figure 3.1 - A Ballot| image:: cluster-images/ballot.png
.. |Figure 3.2 - Replica Role Control Flow| image:: cluster-images/replica.png
.. |Figure 3.3 - Adopted| image:: cluster-images/adopted.png
.. |Figure 3.4 - Accepting| image:: cluster-images/accepting.png
.. |Figure 3.5 - Active| image:: cluster-images/active.png
.. |Figure 3.6 - Bootstrap| image:: cluster-images/bootstrap.png
.. |Figure 3.7 - Scout| image:: cluster-images/leaderscout.png
.. |Figure 3.8 - Commander| image:: cluster-images/leadercommander.png
